 ...
 * License.....: MIT
 *
 * Hashcat mode 36100 — Kalyna-128/256 known-plaintext (DSTU 7624:2014).
 *
 * Hash format: <ciphertext_hex_32>:<plaintext_hex_32>
 *   - both halves are 16 bytes (one Kalyna-128 block) encoded as 32 hex chars
 *   - the candidate password is treated as the raw 16-byte (128-bit) Kalyna key
 *   - the cracker tests Kalyna-128/256(key = pw, pt = $salt) == $digest
 */
...
static const char *HASH_NAME      = "Kalyna-128/256 (PT = $salt, key = $pass)";
...
static const char *ST_PASS        = "hashcat36000kaly";
static const char *ST_HASH        = "fb20d65f525b4ed355b6d44cb5c80dd0:101112131415161718191a1b1c1d1e1f";
...
{
  /* Kalyna-128/256 takes exactly a 32-byte key. */
  return 32;
}
...
{
  /* 32 raw bytes — limit ?b positions so the keyspace fits in 64 bits.
     7 ?b + 25 fixed chars = 32 password bytes. */
