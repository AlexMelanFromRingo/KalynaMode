 ...
 * License.....: MIT
 *
 * Hashcat mode 36000 — Kalyna-128/128 known-plaintext (DSTU 7624:2014).
 *
 ...
 *   - the cracker tests Kalyna-128/128(key = pw, pt = $salt) == $digest
 */
...
static const char *HASH_NAME      = "Kalyna-128/128 (PT = $salt, key = $pass)";
...
{
  /* Kalyna-128/128 takes exactly a 16-byte key. */
  return 16;
}
...
{
  /* 16 raw bytes — limit ?b positions so the keyspace fits in 64 bits.
     7 ?b + 9 fixed chars = 16 password bytes. */
  return "?b?b?b?b?b?b?bxxxxxxxxx";
}
