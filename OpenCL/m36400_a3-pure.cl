 ...
 * License.....: MIT
 *
 * Hashcat mode 36400 — Kalyna-512/512 KPA, attack-mode 3 (mask brute force).
 */
...
DECLSPEC int kalyna_verify_tail_512 (PRIVATE_AS const u32 *ct,
...
KERNEL_FQ KERNEL_FA void m36400_mxx (KERN_ATTR_BASIC ())
{
  const u64 gid = get_global_id (0);
  if (gid >= GID_CNT) return;

  u32 w[KALYNA_KEY_U32];
  for (int i = 0; i < KALYNA_KEY_U32; ++i) w[i] = pws[gid].i[i];
  ...
  for (u32 il_pos = 0; il_pos < IL_CNT; ++il_pos)
  {
    ...
    key[0] = w0;
    for (int i = 1; i < KALYNA_KEY_U32; ++i) key[i] = w[i];
               ...
               BITMAP_MASK, BITMAP_SHIFT1, BITMAP_SHIFT2))
    {
      ...
      if (digest_pos != -1)
      {
        ...
        if (kalyna_verify_tail_512 (ct, &digests_buf[final_hash_pos]))
        {
...
KERNEL_FQ KERNEL_FA void m36400_sxx (KERN_ATTR_BASIC ())
{
  const u64 gid = get_global_id (0);
  if (gid >= GID_CNT) return;
